...
// 비대칭키암호화 : 암호화키 1개(공개키), 복호화키 1개(개인키) -> 두개가쌍으로 있음 -> RSA방식의 암호화 알고리즘
...
public class AESEncryptor {
	...
	public AESEncryptor() {
		// key값을 생성하거나 가져오기
		//1. 생성된 키가 있으면 저장된파일에서 key를 가져와서 key 변수에 저장
		//2. 생성된 키가 없으면 SecretKey클래스를 생성하여 key 변수에 저장
	...
	private void generateKey(File keyFile) { // 키생성하는메소드 (파일에 저장)
		// java.crypto 패키지에서 제공하는 KeyGenerate 클래스를 
	...
	public static String encryptData(String oriData) throws Exception{
		...
		byte[] targetData=oriData.getBytes(Charset.forName("UTF-8")); // 바이트단위로 쪼갬
